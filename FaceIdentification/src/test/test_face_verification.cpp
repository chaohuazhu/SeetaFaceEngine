...
void mean_image_show()
{
	...
	// read image from binary format file
	FILE *fpr = fopen("d:/dl/data/caisa_val.mean", "rb");
	if (fpr == NULL)
	{
		cout << "Open error!" << endl;
		fclose(fpr);
	...
	int rows = 256;
	int cols = 256;
	char* const buf = new char[14];
	fread(buf, sizeof(char), 14, fpr);
	...
	//	fread(&pData[i * 3 + 2], sizeof(char), 1, fpr);
	//}
	float* const char_dst_head_r = new float[256 * 256];
	float* const char_dst_head_g = new float[256 * 256];
	float* const char_dst_head_b = new float[256 * 256];
	fread(char_dst_head_r, sizeof(float), 256 * 256, fpr);
	fread(char_dst_head_g, sizeof(float), 256 * 256, fpr);
	fread(char_dst_head_b, sizeof(float), 256 * 256, fpr);

	for (int i = 0; i<rows*cols; i++)
	{
		pData[i * 3] = int(char_dst_head_b[i]);
		pData[i * 3 + 1] = int(char_dst_head_g[i]);
		pData[i * 3 + 2] = int(char_dst_head_r[i]);
	}
	fclose(fpr);
